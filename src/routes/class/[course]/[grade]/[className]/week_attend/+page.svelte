<script>
  ...
  // controla se sábado deve aparecer
  let saturdayHasClass = false;
  ...
  function getWeekDates(baseDateStr) {
    ...
    const result = {};
    for (let i = 0; i < 6; i++) {   // agora inclui sábado
  ...
  async function loadWeek() {
    ...
    for (const d of days) {
      ...
      // detectar se sábado tem aula (somente curso 集)
      if (d === "土" && course === "集") {
        for (const period of ["１限目","２限目","３限目","４限目","５限目","６限目"]) {
          if (subDay?.[period]?.subject) {
            saturdayHasClass = true;
            break;
          }
        }
      }
  ...
  <div class="week-grid">
    ...
    {#each days.filter(d => d !== "土" || (course === "集" && saturdayHasClass)) as d}
    <div class="day-table">
